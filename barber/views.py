...
class AppointmentsView(viewsets.ModelViewSet):
    ...
    def new_appointment(self, request):
        date_booked_start = datetime.strptime(
            getpost(request, 'date_booked_start'), '%Y-%m-%d %H:%M')
        date_booked_end = datetime.strptime(
            getpost(request, 'date_booked_end'), '%Y-%m-%d %H:%M')
    ...
    def get_free_places(self, request):
        beginweek = datetime.strptime(
            getpost(request, 'beginweek'), '%Y-%m-%d')
        date = beginweek
        ...
        appointment_slices = Appointments.objects.filter(
            time_slice_id=2).values()
        # print('appointments slices', appointment_slices)
        while date <= endweek:
            try:
    ...
    #         getpost(request, 'start_day'), '%d/%m/%Y, %H:%M:%S')
    #     end_day = datetime.strptime(
    #         getpost(request, 'end_day'), '%d/%m/%Y, %H:%M:%S')
    ...
    #         dates.append(
    #             {'date_end': appointment['date_booked_end'], 'date_start': appointment['date_booked_start']})
    ...
    #     start_day = datetime.strptime(
    #         getpost(request, 'start_day'), '%Y-%m-%d')
