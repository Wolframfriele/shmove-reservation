  ...
  <v-main>
    <validation-observer ref="observer" v-slot="{ invalid }">
      <form @submit.prevent="submit">
        <validation-provider v-slot="{ errors }" name="Wachtwoord">
          <v-text-field
            v-model="wachtwoord"
        ...
        <validation-provider v-slot="{ errors }" name="Wachtwoord Herhalen">
          <v-text-field
            v-model="wachtwoord2"
            :error-messages="errors"
...
export default {
  ...
  data: () => ({
    ...
    rules: {
      ...
      passwordmatch: input =>
        input == this.wachtwoord || `De wachtwoorden zijn niet hetzelfde`
    }
  }),
